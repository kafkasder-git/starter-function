...
export function useMessaging(options: MessagingHookOptions = {}): UseMessagingReturn {
    ...
    async (data: CreateConversationData): Promise<Conversation> => {
      try {
        ...
        if (data.type === 'direct') {
          ...
          const existingDirect = existingConversations.find(
            (conv) =>
              conv.type === 'direct' &&
        ...
        const successMessage =
          data.type === 'group' ? `Grup "${data.name}" oluşturuldu` : 'Konuşma oluşturuldu';
  ...
  const addParticipants = useCallback(
    async (conversationId: string, userIds: string[]): Promise<void> => {
      try {
        setLoading(true);
        clearError();
  ...
  const sendMessage = useCallback(
    async (data: SendMessageData): Promise<Message> => {
      try {
    ...
    [selectedConversation, isAuthenticated, user, handleError, setMessages, setConversations]
  );
  ...
  const loadMessages = useCallback(
    async (conversationId?: string, limit?: number, offset?: number): Promise<void> => {
      try {
    ...
    [selectedConversation, messageLimit, handleError, clearError, setLoading, setMessages]
  );
        ...
        prev.map((conv) =>
          conv.id === message.conversationId
            ? {
                ...
                lastMessageAt: message.createdAt,
                unreadCount:
                  message.senderId !== user?.id ? conv.unreadCount + 1 : conv.unreadCount,
              }
            : conv
        )
      );
  ...
  const updateMessageReadStatus = useCallback(
    (messageId: string, readStatus: { userId: string; userName: string; readAt: Date }) => {
      setMessages((prev) =>
        prev.map((msg) =>
          msg.id === messageId
            ? {
