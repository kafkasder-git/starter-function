...
async def run_test():
    ...
    try:
        ...
        # Click on 'Üye Listesi' button to open member list module with search functionality
        frame = context.pages[-1]
        elem = frame.locator('xpath=html/body/div[2]/div/div/div[2]/button').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        

        # Click 'Yeni Üye Ekle' (Add New Member) button to add a test member
        frame = context.pages[-1]
        elem = frame.locator('xpath=html/body/div/div/div[2]/main/div/div/div/div/div/div/div/div[2]/div/button[2]').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        

        # Try clicking the 'İlk Üyeyi Ekle' button as an alternative to add a member or report the issue if it also fails.
        frame = context.pages[-1]
