...
async def run_test():
    ...
    try:
        ...
        # Input username for restricted role and password, then click login
        ...
        # Input valid restricted role user credentials (e.g., isahamid095@gmail.com) and password, then click login
        frame = context.pages[-1]
        elem = frame.locator('xpath=html/body/div/div/div[2]/div/div[2]/form/div/div/input').nth(0)
        ...
        # Attempt to access an admin-only module or perform a privileged action to verify access restrictions
        frame = context.pages[-1]
        elem = frame.locator('xpath=html/body/div/div/div[2]/div/aside/div/div/div[10]/div/button').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        

        # Click on 'Hukuki Danışmanlık' (index 48) to verify if access is denied or allowed
        frame = context.pages[-1]
