...
async def run_test():
    ...
    try:
        ...
        # Click on 'Yeni İhtiyaç Sahibi' (New Beneficiary) button to add a new beneficiary
        frame = context.pages[-1]
        elem = frame.locator('xpath=html/body/div/div/div[2]/main/div/div/div/div/div[3]/div/div[2]/div/div/div/div[2]/div/button').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        

        # Click on 'Yeni İhtiyaç Sahibi Ekle' button to start adding a new beneficiary
        frame = context.pages[-1]
        ...
        # Navigate to 'Yardım Talepleri' (Aid Applications) page to submit a new aid application for the beneficiary
        frame = context.pages[-1]
