...
async def run_test():
    ...
    try:
        ...
        # Click 'Yeni İhtiyaç Sahibi' button to start new beneficiary registration
        frame = context.pages[-1]
        elem = frame.locator('xpath=html/body/div/div/div[2]/main/div/div/div/div/div[3]/div/div[2]/div/div/div/div[2]/div/button').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        

        # Click 'Yeni İhtiyaç Sahibi Ekle' button to start new beneficiary registration
        frame = context.pages[-1]
        elem = frame.locator('xpath=html/body/div/div/div[2]/main/div/div/div/div/div/div/div/div[2]/div/button[2]').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        

        # Fill in all mandatory fields in the beneficiary registration form with valid test data and submit the form.
        frame = context.pages[-1]
        ...
        # Click 'Ekle' button to upload required documents for the beneficiary.
        frame = context.pages[-1]
