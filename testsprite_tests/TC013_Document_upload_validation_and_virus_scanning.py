...
async def run_test():
    ...
    try:
        ...
        # Click on 'İhtiyaç Sahipleri' (Beneficiaries) button to access beneficiary management
        frame = context.pages[-1]
        elem = frame.locator('xpath=html/body/div/div/div[2]/main/div/div/div/div/div[2]/div[3]/button').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        

        # Try clicking 'Yeni İhtiyaç Sahibi' button as alternative to access beneficiary document upload or report issue if no navigation possible.
        frame = context.pages[-1]
