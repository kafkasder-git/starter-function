...
async def run_test():
    ...
    try:
        ...
        # Navigate to the large data listing page, e.g. beneficiaries or donations
        frame = context.pages[-1]
        elem = frame.locator('xpath=html/body/div/div/div[2]/main/div/div/div/div/div[2]/div[3]/button').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        

        # Try clicking the 'Genel Bakış' tab (index 34) or other sidebar navigation elements to find the large data listing page.
        frame = context.pages[-1]
        elem = frame.locator('xpath=html/body/div/div/div[2]/main/div/div/div/div/div[3]/div/div/button').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        

        # Try clicking the 'Başvuru Onayları' button (index 39) or 'Yeni İhtiyaç Sahibi' button (index 38) to access large data listing or management pages.
        frame = context.pages[-1]
