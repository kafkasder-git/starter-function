...
async def run_test():
    ...
    try:
        ...
        # Click 'Yeni Üye Ekle' (New Member Add) button to open the new member form
        frame = context.pages[-1]
        elem = frame.locator('xpath=html/body/div/div/div[2]/main/div/div/div/div/div/div/div/div[2]/div/button[2]').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        

        # Click the 'İlk Üyeyi Ekle' button to try opening the new member form to trigger data change event.
        frame = context.pages[-1]
