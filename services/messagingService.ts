...
export class MessagingService {
  ...
  async createConversation(data: CreateConversationData): Promise<Conversation> {
    try {
      ...
      if (conversationError) {
        throw new MessagingServiceError('CONVERSATION_CREATE_FAILED', 'Failed to create conversation', conversationError);
      }
      ...
      if (!fullConversation) {
        throw new MessagingServiceError('CONVERSATION_NOT_FOUND', 'Conversation created but could not be retrieved');
      }
  ...
  async getConversations(filters: ConversationFilters): Promise<Conversation[]> {
    try {
      ...
      if (participantError) {
        throw new MessagingServiceError('PARTICIPANTS_FETCH_FAILED', 'Failed to fetch user participants', participantError);
      }
      ...
      const { data: conversationData, error: conversationError } = await db.list(
        ...
        [
          ...conversationQueries,
          queryHelpers.orderDesc('last_message_at')
        ]
      );

      if (conversationError) {
        throw new MessagingServiceError('CONVERSATIONS_FETCH_FAILED', 'Failed to fetch conversations', conversationError);
      }
  ...
  async getConversationById(conversationId: string): Promise<Conversation | null> {
    try {
      ...
      const fullConversation = await this.buildConversationFromDocument(conversation, currentUser.id);
      return fullConversation;
  ...
  async addParticipants(conversationId: string, userIds: string[]): Promise<void> {
    try {
      ...
      if (errors.length > 0) {
        ...
        throw new MessagingServiceError('PARTICIPANTS_ADD_FAILED', 'Some participants could not be added');
      }
  ...
  async removeParticipant(conversationId: string, userId: string): Promise<void> {
    try {
      ...
      if (participantError || !participantData?.documents?.length) {
        throw new MessagingServiceError('PARTICIPANT_NOT_FOUND', 'Participant not found in conversation');
      }
      ...
      if (updateError) {
        throw new MessagingServiceError('PARTICIPANT_REMOVE_FAILED', 'Failed to remove participant', updateError);
      }
  ...
  async sendMessage(data: SendMessageData): Promise<Message> {
    try {
      ...
      if (messageError) {
        throw new MessagingServiceError('MESSAGE_CREATE_FAILED', 'Failed to create message', messageError);
      }
  ...
  async getMessages(filters: MessageFilters): Promise<Message[]> {
    try {
      ...
      const { data: messageData, error } = await db.list(
        collections.MESSAGES,
        queries
      );
  ...
  async markAsRead(messageId: string): Promise<void> {
    try {
      ...
      const { data: existingRead, error: existingError } = await db.list(
        ...
        [
          queryHelpers.equal('message_id', messageId),
      ...
      if (createError) {
        throw new MessagingServiceError('READ_STATUS_CREATE_FAILED', 'Failed to mark message as read', createError);
      }
  ...
  async deleteMessage(messageId: string): Promise<void> {
    try {
      ...
      if (updateError) {
        throw new MessagingServiceError('MESSAGE_DELETE_FAILED', 'Failed to delete message', updateError);
      }
  ...
  async getUnreadCount(conversationId: string): Promise<number> {
    try {
      ...
      const { data: messageData, error: messageError } = await db.list(
        collections.MESSAGES,
        [
          ...messageQueries,
    ...
    try {
      ...
      if (existingError || !existingIndicator?.documents?.length) {
        ...
        if (isTyping) {
          ...
          await db.update(
            collections.TYPING_INDICATORS,
            existingIndicator.documents[0].$id,
            {
              is_typing: true,
              updated_at: new Date().toISOString()
            }
   ...
   */
  private async checkConversationPermission(conversationId: string, userId: string, requiredRole: 'admin' | 'member'): Promise<void> {
    const { data: participantData, error } = await db.list(
      collections.CONVERSATION_PARTICIPANTS,
      [
        queryHelpers.equal('conversation_id', conversationId),
        queryHelpers.equal('user_id', userId),
    ...
    if (error || !participantData?.documents?.length) {
      throw new MessagingServiceError('ACCESS_DENIED', 'User is not a participant in this conversation');
    }
   ...
   */
  private async buildConversationFromDocument(doc: ConversationDocument, _currentUserId: string): Promise<Conversation | null> {
    try {
      // Get participants
      const { data: participantData, error: participantError } = await db.list(
        collections.CONVERSATION_PARTICIPANTS,
      ...
      const participantPromises = participantData.documents.map(async (participant) => {
        const { data: userProfile } = await db.get(
          collections.USER_PROFILES,
          participant.user_id
        );

        // Get presence status
        const { data: presenceData } = await db.list(
          collections.USER_PRESENCE,
   ...
   */
  protected async buildMessageFromDocument(doc: MessageDocument, attachments: any[] = []): Promise<Message | null> {
    try {
      // Get sender profile
